# Software Carbon Intensity for AI and EU AI Act Environmental Compliance

## A Framework for Measuring and Reducing AI's Climate Impact
...
Where `O = E × I` represents operational emissions.
...
- **I** (Carbon intensity) \= Average or marginal grid carbon intensity in gCO2eq/kWh  
- **M** (Embodied emissions) \= Hardware lifecycle emissions in gCO2eq  
...
```
O = E × I = 150,000 kWh × 400 gCO2eq/kWh = 60,000,000 gCO2eq = 60,000 kgCO2eq
```

**Embodied emissions (M)**:

```
M per server = 2,500 × (720 / 35,040) × 1 = 51.4 kgCO2eq
M (total for 8 servers) = 51.4 × 8 = 411 kgCO2eq
```

**Total emissions**:

```
Total = O + M = 60,000 + 411 = 60,411 kgCO2eq
```

**SCI per training token**:

```
SCI = 60,411 kgCO2eq / (2 × 10^12 tokens) = 0.0000302 gCO2eq/token
    = 30.2 µgCO2eq/training token
```

*Note: The embodied emissions figure of 2,500 kgCO2eq per server is illustrative. Actual values vary significantly by manufacturer and configuration, and such data is often unavailable.*
...
```
O = 2,400 × 250 = 600,000 gCO2eq = 600 kgCO2eq
```

**Daily embodied emissions**:

```
M = 1,500 × (24 / 35,040) × 1 = 1.03 kgCO2eq per server
M (total, 4 servers) = 4.1 kgCO2eq
```

**Daily SCI per token**:

```
SCI = (600 + 4.1) kgCO2eq / 500,000,000 tokens
    = 1.21 mgCO2eq/token
```
...
7. **ISO/IEC 21031:2024** — Software Carbon Intensity specification [https://www.iso.org/standard/86612.html](https://www.iso.org/standard/86612.html)  
     
8. **SCI Specification** — Green Software Foundation [https://sci.greensoftware.foundation/](https://sci.greensoftware.foundation/)  
     
9. **SCI for AI Specification** [https://github.com/Green-Software-Foundation/sci-ai/blob/main/SPEC.md](https://github.com/Green-Software-Foundation/sci-ai/blob/main/SPEC.md)

### Analysis and Commentary

10. **White & Case** — Energy Efficiency Requirements Under the EU AI Act [https://www.whitecase.com/insight-alert/energy-efficiency-requirements-under-eu-ai-act](https://www.whitecase.com/insight-alert/energy-efficiency-requirements-under-eu-ai-act)  
      
11. **Heinrich Böll Foundation** — EU AI Act: A Missed Opportunity for Environmental Sustainability [https://eu.boell.org/en/2024/04/08/eu-ai-act-missed-opportunity](https://eu.boell.org/en/2024/04/08/eu-ai-act-missed-opportunity)
